...
export default function InstitutionStudents(){
    ...
    useEffect(() => {
        getUserName().then(resp => {
            ...
            fetch(`http://localhost:3333/teacher/${resp.id}`)
            .then(json => json.json())
            .then(data => {
                
                fetch(`http://localhost:3333/instituition/${data.institutionID}/students`)
                .then(json => json.json())
    ...
    return(
        <div className="h-fit w-full flex flex-col gap-4">
...

            {
                students.map(aluno => {
                    if(searchByName == "")
                        return <TeacherStudentContainer addStudent key={aluno.studentID} studentID={aluno.studentID+""} teacherID={id+""} avatarURL={aluno.avatarURL} nomeAluno={aluno.studentName} serieAluno={materiasNames[Number(aluno.studentGrade)]} />
